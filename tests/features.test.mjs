...
const [api, form, imageUpload, stock, users, settings, receipt, receiptPdf, payment, minimized, pdv, reports, layout, googleImages, css] = await Promise.all([
  read('api/index.js'),
...
assert.match(api, /revokedPasswordHash/, 'A exclusão deve revogar a senha anterior.');
assert.match(api, /sql\.transaction\(tx => \[/, 'Operações críticas devem usar transação.');
assert.match(settings, /Zerar dados de uma tela/, 'Configurações deve oferecer limpeza seletiva.');
assert.match(receipt, /downloadSaleReceiptPdf/, 'O modal deve usar o gerador compartilhado de recibos.');
assert.match(receiptPdf, /Unitário:/, 'O recibo deve exibir preço unitário sem caracteres quebrados.');
assert.doesNotMatch(receiptPdf, /→/, 'O PDF não deve usar o símbolo que gerava falhas visuais.');
assert.match(payment, /input\.focus\(\)/, 'O campo de valor deve receber foco.');
assert.match(minimized, /DragDropContext/, 'Vendas minimizadas devem permitir reordenação.');
assert.match(pdv, /PDV_DRAFT_INACTIVITY_MS/, 'O PDV deve controlar o limite de inatividade.');
...
assert.match(css, /--font-body:\s*'Montserrat'/, 'A fonte Montserrat deve permanecer aplicada.');
