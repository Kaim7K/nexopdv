...
assert.match(db, /CURRENT_SCHEMA_VERSION = 19/, 'A aplicação deve exigir a migração mais recente.');
...
assert.match(api, /cashClosingEntryQuery/, 'Entradas no fechamento também precisam ser sincronizadas com o Financeiro.');
...
assert.match(api, /\['sales', 'cash_sessions'\]\.includes\(table\)/, 'Vendas e caixas não podem ser alterados por rotas genéricas sem estorno.');
...
assert.match(finance, /buildCashSessionSummary/, 'Conciliação e fechamento devem compartilhar o mesmo agregador.');
assert.match(cashSummary, /CANCELLED_MOVEMENT_STATUSES/, 'Conciliação deve ignorar movimentos cancelados e estornados.');
