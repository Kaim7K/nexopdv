    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="grid grid-cols-2 gap-1.5 border-b border-border px-2 py-1.5 sm:block sm:space-y-2 sm:px-3 sm:py-3">
        ...
        <select
          ...
          className={`h-9 min-w-0 rounded-lg border border-border bg-background px-2 text-xs font-medium outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 sm:hidden ${categories.length ? '' : 'col-span-2'}`}
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain p-1.5 sm:p-3">
        {visibleProducts.length === 0 ? (
          ...
          <div className="grid gap-1.5 [grid-template-columns:repeat(auto-fill,minmax(5.8rem,1fr))] sm:gap-2 sm:[grid-template-columns:repeat(auto-fill,minmax(6.2rem,1fr))]">
            {visibleProducts.map(product => (
              ...
              >
                ...
                <div title={product.name} className="mb-0.5 line-clamp-1 text-[10px] font-semibold leading-[0.875rem] sm:mb-1 sm:line-clamp-2 sm:min-h-8 sm:text-xs sm:leading-4">{product.name}</div>
                <div className="mt-auto flex min-w-0 items-center justify-between gap-1 sm:flex-col sm:items-stretch sm:gap-0.5">
                ...
                {sortMode === 'sold_desc' && (
                  <div className="mt-1 hidden min-w-0 items-center gap-1 text-[10px] leading-3 text-muted-foreground sm:flex">
