    ...
    <section className="rounded-2xl border border-border bg-card p-5 sm:p-6 lg:col-span-12" aria-labelledby="stock-alert-title">
      <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_minmax(560px,650px)] 2xl:items-start">
        ...
        <div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1.35fr)_minmax(140px,.8fr)_minmax(150px,.85fr)]">
          <label className="flex min-h-[62px] cursor-pointer items-center justify-between gap-3 rounded-xl border border-border bg-muted/20 px-3.5 py-2.5 text-sm font-semibold transition hover:bg-muted/40"><span className="leading-5">Ativar envio automático</span><input type="checkbox" checked={data.enabled !== false} disabled={busy === 'settings'} onChange={event => saveSettings({ ...data, enabled:event.target.checked })} className="h-5 w-5 shrink-0 accent-[var(--market-primary)]" /></label>
          <label className="grid min-w-0 gap-1 text-xs font-semibold text-muted-foreground"><span className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" /> Horário</span><input type="time" value="20:00" disabled className="h-10 w-full min-w-0 rounded-xl border border-border bg-muted px-3 text-sm text-foreground opacity-70" title="No plano Hobby da Vercel, o agendamento gratuito pode executar apenas uma vez por dia." /></label>
          <label className="grid min-w-0 gap-1 text-xs font-semibold text-muted-foreground sm:col-span-2 lg:col-span-1"><span>Período</span><select value={data.frequency || 'daily'} disabled={busy === 'settings' || data.enabled === false} onChange={event => saveSettings({ ...data, frequency:event.target.value })} className="h-10 w-full min-w-0 rounded-xl border border-border bg-background px-3 text-sm text-foreground disabled:bg-muted disabled:opacity-60"><option value="daily">Diário</option><option value="weekly">Semanal</option><option value="fortnightly">Quinzenal</option><option value="monthly">Mensal</option></select></label>
        </div>
      </div>
