  ...
  return page.evaluate(() => {
    ...
    const oversized = [
      ...document.querySelectorAll(
        'main, section, article, form, aside, header, [role="dialog"], button, input, select, textarea',
      ),
    ]
      .map((element) => {
        const rect = element.getBoundingClientRect();
        ...
        const isControl = ["button", "input", "select", "textarea"].includes(
          tag,
        );
        const isLayoutShell =
          ["main", "aside"].includes(tag) ||
          (tag === "header" && styles.position === "sticky");
        const tooTall =
          (!isLayoutShell &&
            isControl &&
            rect.height > Math.max(64, window.innerHeight * 0.11)) ||
          (!isLayoutShell &&
            !isControl &&
        ...
        const tooWide =
          rect.width > window.innerWidth + 2 && styles.position !== "fixed";
    ...
    return {
      ...
      oversized,
      appCrashed: Boolean(
        document.body?.textContent?.includes("A tela encontrou um problema"),
      ),
    };
  });
}
...
const page =
  context
    .pages()
    .find((candidate) => candidate.url().startsWith(targetOrigin)) ||
  context.pages()[0] ||
...
const loggedIn =
  email || password ? await login(page) : !page.url().includes("/login");
const results = [];
...
const report = {
  ...
  captureFullPage,
  routes,
  viewports: viewports.map(([label, width, height]) => ({
    label,
    width,
    height,
  })),
...
for (const issue of issues.slice(0, 20)) {
  console.log(
    `- ${issue.viewport} ${issue.route}: overflow=${issue.horizontalOverflow}, crash=${issue.appCrashed}, grandes=${issue.oversized.length}, captura=${issue.screenshotError ? "falhou" : "ok"}`,
  );
}
