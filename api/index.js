...
import { getStockEmailConfiguration, isValidAlertEmail, loadMarketEmailBrand, loadStockAlertReport, sendStockAlertEmail } from '../server/stock-alerts.js';
...
async function processScheduledStockAlerts(sql, now = new Date()) {
  ...
  for (const market of markets) {
    ...
    try {
      ...
      const sent = await sendStockAlertEmail({ to:recipients, marketName:market.name, brand, products, generatedAt:now.toISOString() });
...
async function getCashSessionSummary(sql, marketId, session) {
  ...
  const movementRows = await sql`SELECT id,data,created_date,updated_date FROM nexo.records WHERE market_id=${marketId} AND entity='cash_movements' AND data->>'cash_session_id'=${session.id} ORDER BY created_date ASC`;
  const movements = movementRows.map(recordFromRow);
  const entries = roundMoney(movements.filter(item => item.type === 'entrada').reduce((sum,item) => sum + Number(item.amount || 0), 0));
  const withdrawals = roundMoney(movements.filter(item => item.type === 'retirada').reduce((sum,item) => sum + Number(item.amount || 0), 0));
  ...
  return {
    ...
    entries,
    withdrawals,
    expected_cash: roundMoney(openingAmount + cashSales + entries - withdrawals),
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>${location}</loc><changefreq>weekly</changefreq><priority>1.0</priority></url></urlset>`;
  ...
  if (path[0] === 'auth' && path[1] === 'login') {
    ...
    if (authenticated.role !== 'super_admin' && authenticated.maintenance_mode) throw new AppError(503,'PLATFORM_MAINTENANCE',authenticated.maintenance_message || 'A plataforma está em manutenção. Tente novamente em instantes.');
    await sql`UPDATE nexo.users SET last_login_at = now() WHERE id = ${authenticated.id}`;
    const [sessionPolicy] = await sql`SELECT value #>> '{}' AS hours FROM nexo.platform_settings WHERE key='security_session_hours'`;
  ...
  if (user.role !== 'super_admin' && user.maintenance_mode) return send(res,503,{ code:'PLATFORM_MAINTENANCE',message:user.maintenance_message || 'A plataforma está em manutenção. Tente novamente em instantes.' });
  ...
  if (path[0] === 'stock-alerts') {
    ...
    if (path[1] === 'test' && req.method === 'POST') {
      ...
      const sent = await sendStockAlertEmail({ to:[email], marketName:user.market_name || 'Nexo PDV', brand, products, generatedAt:new Date().toISOString() });
  ...
  if (path[0] === 'products' && path[1] === 'quick') {
    if (req.method !== 'POST') return methodNotAllowed(res, ['POST']);
    if (!user.market_id || !['vendedor','gerente','admin'].includes(user.role)) return send(res, 403, { message:'Sem permissão para cadastrar produtos no PDV.' });
    if (!(user.enabled_modules || []).includes('pdv')) return send(res, 403, { message:'O PDV não está habilitado para este mercadinho.' });
    const barcode = text(req.body.barcode, 64);
    const name = text(req.body.name, 180);
    if (!/^\d{6,32}$/.test(barcode)) return send(res, 400, { message:'O código de barras lido não é válido.' });
    if (!name) return send(res, 400, { message:'Informe o nome do produto.' });
    const configRows = await sql`SELECT data FROM nexo.records WHERE market_id=${user.market_id} AND entity='system_configs' AND data->>'key'=ANY(ARRAY['default_product_unit','default_product_sale_price','default_product_category','default_product_track_stock'])`;
    const [capacity] = await sql`SELECT market.product_limit,(SELECT count(*)::int FROM nexo.records WHERE market_id=market.id AND entity='products') AS product_count FROM nexo.markets market WHERE market.id=${user.market_id}`;
    if (capacity?.product_limit && Number(capacity.product_count) >= Number(capacity.product_limit)) return send(res,409,{ code:'PRODUCT_LIMIT_REACHED',message:'O limite de produtos deste mercadinho foi atingido. Ajuste o plano antes de cadastrar outro item.' });
    const defaults = Object.fromEntries(configRows.map(row => [row.data?.key,row.data?.value]));
    const unit = PRODUCT_UNITS.has(defaults.default_product_unit) ? defaults.default_product_unit : 'unidade';
    const defaultPrice = roundMoney(Math.max(0,Number(defaults.default_product_sale_price || 0)));
    const productPayload = {
      name, barcode,
      internal_code: `PDV-${randomUUID().replace(/-/g,'').slice(0,12).toUpperCase()}`,
      image_url:'', sale_price:Number.isFinite(defaultPrice) ? defaultPrice : 0,
      cost_price:null, quantity:0, unit, status:'ativo',
    ...
    const auditPayload = {
      action_type:'produto_cadastrado_rapido', entity_type:'product',
    ...
    `;
    if (rows[0]) return send(res, 201, { product:recordFromRow(rows[0]), created:true });
    const [existing] = await sql`SELECT id,data,created_date,updated_date FROM nexo.records WHERE market_id=${user.market_id} AND entity='products' AND btrim(data->>'barcode')=${barcode} LIMIT 1`;
    if (!existing) throw new AppError(409,'PRODUCT_CREATE_CONFLICT','O produto foi alterado por outro usuário. Leia o código novamente.');
  ...
  if (path[0] === 'cash') {
    ...
    if (path[1] === 'history' && req.method === 'GET') {
      ...
      const sellerId = user.role === 'vendedor' ? user.id : text(req.query.operator_id,64);
      ...
      for (const row of rows) {
        ...
        const summary = session.status === 'aberto' ? await getCashSessionSummary(sql,user.market_id,session) : (session.summary || {});
        items.push({ ...session, summary, total_sales:Number(summary.total || 0), entries:Number(summary.entries || 0), withdrawals:Number(summary.withdrawals || 0), payments:summary.payments || {}, final_amount:session.closing_amount ?? summary.expected_cash ?? 0 });
      }
      const [operatorRows,unitRows] = await Promise.all([
        user.role === 'vendedor' ? [{ id:user.id,name:user.full_name || user.email }] : sql`SELECT id,COALESCE(full_name,email) AS name FROM nexo.users WHERE market_id=${user.market_id} ORDER BY COALESCE(full_name,email)`,
        sql`SELECT id,name,code,active FROM nexo.market_units WHERE market_id=${user.market_id} ORDER BY name`,
      ]);
      const total = Number(rows[0]?.total_count || 0);
      return send(res,200,{ items,page,page_size:pageSize,total,page_count:Math.max(1,Math.ceil(total/pageSize)),operators:operatorRows,units:unitRows });
    }

    if (isUuid(path[1]) && !path[2] && req.method === 'GET') {
      const [row] = await sql`SELECT id,data,created_date,updated_date FROM nexo.records WHERE id=${path[1]} AND market_id=${user.market_id} AND entity='cash_sessions' AND (${user.role !== 'vendedor'} OR data->>'seller_id'=${user.id})`;
      if (!row) return send(res,404,{ message:'Caixa não encontrado ou sem permissão.' });
      const session = recordFromRow(row);
      return send(res,200,{ session,summary:await getCashSessionSummary(sql,user.market_id,session) });
    }
    ...
    if (isUuid(path[1]) && path[2] === 'movements') {
      ...
      if (!['entrada','retirada'].includes(type)) return send(res,400,{ message:'Selecione entrada ou retirada.' });
      if (!Number.isFinite(amount) || amount <= 0 || amount > 10_000_000) return send(res,400,{ message:'Informe um valor de movimentação válido.' });
      const [sessionRow] = await sql`SELECT id,data,created_date,updated_date FROM nexo.records WHERE id=${path[1]} AND market_id=${user.market_id} AND entity='cash_sessions' AND data->>'status'='aberto' AND data->>'seller_id'=${user.id}`;
      if (!sessionRow) return send(res,409,{ message:'Somente o operador responsável pode movimentar o próprio caixa aberto.' });
      const payload = { cash_session_id:path[1],type,amount,note,operator_id:user.id,operator_name:user.full_name || user.email,created_at:new Date().toISOString() };
      const [movement] = await sql`WITH movement AS (INSERT INTO nexo.records(market_id,entity,data) VALUES(${user.market_id},'cash_movements',${JSON.stringify(payload)}::jsonb) RETURNING id,data,created_date,updated_date),audit AS (INSERT INTO nexo.records(market_id,entity,data) SELECT ${user.market_id},'general_audits',jsonb_build_object('action_type','movimentacao_caixa','entity_type','cash_session','entity_id',${path[1]}::uuid,'user_id',${user.id},'user_name',${user.full_name || user.email},'description',${type === 'entrada' ? 'Entrada registrada no caixa' : 'Retirada registrada no caixa'},'details',${JSON.stringify({type,amount,note})}::jsonb) FROM movement) SELECT * FROM movement`;
      const session = recordFromRow(sessionRow);
      return send(res,201,{ movement:recordFromRow(movement),summary:await getCashSessionSummary(sql,user.market_id,session) });
    }
    ...
    if (path[1] === 'close') {
      ...
      const { sales: cashSalesDetail, movements: cashMovementsDetail, filters: cashFilters, ...summarySnapshot } = summary;
      const update = {
        status: 'fechado',
        closed_at: closedAt,
      ...
      return send(res, 200, { session: recordFromRow(row), summary: { ...summary, sales: cashSalesDetail, movements: cashMovementsDetail, filters: { ...(cashFilters || {}), to: closedAt }, closing_amount: closingAmount, difference: update.difference } });
    }
  ...
  if (path[0] === 'admin') {
    if (user.role !== 'super_admin') return send(res,403,{ message:'Acesso restrito ao Super Admin.' });
    ...
    if (path[1] === 'overview' && req.method === 'GET') {
      ...
      return send(res,200,{ metrics:{ ...metrics[0],estimated_revenue:Number(metrics[0]?.estimated_revenue || 0),processed_volume:Number(metrics[0]?.processed_volume || 0) },market_growth:marketGrowth });
    }

    if (path[1] === 'plans') {
      if (req.method === 'GET') {
        const plans = await sql`SELECT plan.*,count(subscription.id)::int AS subscription_count FROM nexo.plans plan LEFT JOIN nexo.subscriptions subscription ON subscription.plan_id=plan.id AND subscription.status IN ('teste','ativa','inadimplente','suspensa') GROUP BY plan.id ORDER BY plan.monthly_price,plan.name`;
        return send(res,200,plans.map(plan => ({ ...plan,monthly_price:Number(plan.monthly_price) })));
      }
      const planId = path[2];
      if (!['POST','PATCH'].includes(req.method)) return methodNotAllowed(res,['GET','POST','PATCH']);
      if (req.method === 'PATCH' && !isUuid(planId)) return send(res,400,{ message:'Plano inválido.' });
      ...
      const modules = Array.isArray(source.enabled_modules) ? source.enabled_modules : [];
      if (!name || !Number.isFinite(monthlyPrice) || monthlyPrice < 0 || trialDays < 0 || trialDays > 365) return send(res,400,{ message:'Revise nome, preço e período de teste do plano.' });
      if (modules.some(module => !MARKET_MODULES.includes(module))) return send(res,400,{ message:'O plano contém funcionalidades inválidas.' });
      const limit = value => value === '' || value === null || value === undefined ? null : Math.max(1,Number.parseInt(value,10) || 1);
      const values = { name,description,monthlyPrice:roundMoney(monthlyPrice),trialDays,userLimit:limit(source.user_limit),productLimit:limit(source.product_limit),unitLimit:limit(source.unit_limit),modules,active:source.active !== false };
      if (req.method === 'POST') {
        const [plan] = await sql`INSERT INTO nexo.plans(name,description,monthly_price,trial_days,user_limit,product_limit,unit_limit,enabled_modules,active) VALUES(${values.name},${values.description},${values.monthlyPrice},${values.trialDays},${values.userLimit},${values.productLimit},${values.unitLimit},${JSON.stringify(values.modules)}::jsonb,${values.active}) RETURNING *`;
        return send(res,201,{ ...plan,monthly_price:Number(plan.monthly_price) });
      }
      const [plan] = await sql`UPDATE nexo.plans SET name=${values.name},description=${values.description},monthly_price=${values.monthlyPrice},trial_days=${values.trialDays},user_limit=${values.userLimit},product_limit=${values.productLimit},unit_limit=${values.unitLimit},enabled_modules=${JSON.stringify(values.modules)}::jsonb,active=${values.active},updated_date=now() WHERE id=${planId} RETURNING *`;
      return send(res,plan?200:404,plan ? { ...plan,monthly_price:Number(plan.monthly_price) } : { message:'Plano não encontrado.' });
    }

    if (path[1] === 'subscriptions') {
      if (req.method === 'GET') {
        const rows = await sql`SELECT subscription.*,market.name AS market_name,plan.name AS plan_name FROM nexo.subscriptions subscription JOIN nexo.markets market ON market.id=subscription.market_id LEFT JOIN nexo.plans plan ON plan.id=subscription.plan_id ORDER BY subscription.updated_date DESC`;
        return send(res,200,rows.map(row => ({ ...row,monthly_price:Number(row.monthly_price) })));
      }
      if (req.method !== 'PATCH' || !isUuid(path[2])) return methodNotAllowed(res,['GET','PATCH']);
      const status = text(req.body.status,30);
      if (!['teste','ativa','inadimplente','cancelada','suspensa'].includes(status)) return send(res,400,{ message:'Status de assinatura inválido.' });
      const reason = text(req.body.cancellation_reason,500);
      const [subscription] = await sql`UPDATE nexo.subscriptions SET status=${status},cancelled_at=CASE WHEN ${status}='cancelada' THEN now() ELSE NULL END,cancellation_reason=CASE WHEN ${status}='cancelada' THEN ${reason} ELSE NULL END,updated_date=now() WHERE id=${path[2]} RETURNING *`;
      if (!subscription) return send(res,404,{ message:'Assinatura não encontrada.' });
      const marketStatus = status === 'cancelada' ? 'cancelado' : status === 'suspensa' || status === 'inadimplente' ? 'suspenso' : status === 'teste' ? 'teste' : 'ativo';
      ...
      return send(res,200,{ ...subscription,monthly_price:Number(subscription.monthly_price) });
    }

    if (path[1] === 'payments') {
      if (req.method === 'GET') {
        const subscriptionId = text(req.query.subscription_id,64);
        const rows = await sql`SELECT payment.*,market.name AS market_name,plan.name AS plan_name FROM nexo.subscription_payments payment JOIN nexo.subscriptions subscription ON subscription.id=payment.subscription_id JOIN nexo.markets market ON market.id=subscription.market_id LEFT JOIN nexo.plans plan ON plan.id=subscription.plan_id WHERE (${subscriptionId === ''} OR payment.subscription_id::text=${subscriptionId}) ORDER BY payment.due_date DESC LIMIT 1000`;
        return send(res,200,rows.map(row => ({ ...row,amount:Number(row.amount) })));
      }
      if (req.method !== 'POST') return methodNotAllowed(res,['GET','POST']);
      if (!isUuid(req.body.subscription_id)) return send(res,400,{ message:'Assinatura inválida.' });
      const amount = roundMoney(Number(req.body.amount));
      const dueDate = parseDateQuery(req.body.due_date);
      const paymentStatus = ['pendente','pago','vencido','estornado'].includes(req.body.status) ? req.body.status : 'pendente';
      if (!Number.isFinite(amount) || amount < 0 || !dueDate) return send(res,400,{ message:'Informe valor e vencimento válidos.' });
      const [payment] = await sql`INSERT INTO nexo.subscription_payments(subscription_id,amount,due_date,paid_at,status,notes) VALUES(${req.body.subscription_id},${amount},${dueDate.toISOString().slice(0,10)},${paymentStatus === 'pago' ? new Date() : null},${paymentStatus},${text(req.body.notes,500)}) RETURNING *`;
      return send(res,201,{ ...payment,amount:Number(payment.amount) });
    }

    if (path[1] === 'reports' && req.method === 'GET') {
      const from = parseDateQuery(req.query.from,new Date(Date.now()-365*86_400_000));
      ...
      return send(res,200,{ growth,revenue:revenue.map(row=>({...row,revenue:Number(row.revenue)})),cancellations,usage:usage.map(row=>({...row,sales_volume:Number(row.sales_volume)})) });
    }

    if (path[1] === 'settings') {
      const allowed = new Set(['email_provider','email_from_name','notification_defaults','plan_enforcement','global_user_limit','global_product_limit','global_unit_limit','security_session_hours','security_login_attempts','integrations','maintenance_mode','maintenance_message','platform_notice']);
      if (req.method === 'GET') {
        const rows = await sql`SELECT key,value,updated_date FROM nexo.platform_settings ORDER BY key`;
        return send(res,200,Object.fromEntries(rows.map(row => [row.key,{ value:row.value,updated_date:row.updated_date }])));
      }
      ...
      if (!entries.length) return send(res,400,{ message:'Nenhuma configuração válida foi informada.' });
      for (const [key,value] of entries) await sql`INSERT INTO nexo.platform_settings(key,value,updated_by) VALUES(${key},${JSON.stringify(value)}::jsonb,${user.id}) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_by=excluded.updated_by,updated_date=now()`;
  ...
  if (path[0] === 'markets') {
    ...
    if (req.method === 'GET' && isUuid(path[1])) {
      const [marketRows,units,history,subscriptions,payments] = await Promise.all([
      ...
      if (!marketRows[0]) return send(res,404,{ message:'Mercadinho não encontrado.' });
      return send(res,200,{ market:{ ...marketRows[0],plan_price:Number(marketRows[0].plan_price || 0) },units,history,subscriptions:subscriptions.map(item=>({...item,monthly_price:Number(item.monthly_price)})),payments:payments.map(item=>({...item,amount:Number(item.amount)})) });
    }
    if (req.method === 'GET') return send(res, 200, await sql`SELECT market.id,market.name,market.slug,market.logo_url,market.primary_color,market.secondary_color,market.enabled_modules,market.require_cash_register,market.active,market.status,market.plan_id,plan.name AS plan_name,market.trial_ends_at,market.subscription_due_date,market.user_limit,market.product_limit,market.unit_limit,market.created_date,(SELECT count(*)::int FROM nexo.users WHERE market_id=market.id AND active) AS user_count FROM nexo.markets market LEFT JOIN nexo.plans plan ON plan.id=market.plan_id ORDER BY market.name`);
    if (req.method === 'POST') {
    ...
    if (req.method === 'PATCH') {
      ...
      const status = ['ativo','suspenso','teste','cancelado'].includes(b.status) ? b.status : (typeof b.active === 'boolean' ? (b.active ? 'ativo' : 'suspenso') : null);
      const planId = b.plan_id === null ? null : isUuid(b.plan_id) ? b.plan_id : undefined;
      const [market] = await sql`WITH changed AS (UPDATE nexo.markets SET name=COALESCE(${updatedName},name),logo_url=COALESCE(${logoUrl},logo_url),primary_color=COALESCE(${b.primary_color || null},primary_color),secondary_color=COALESCE(${b.secondary_color || null},secondary_color),enabled_modules=COALESCE(${b.enabled_modules ? JSON.stringify(b.enabled_modules) : null}::jsonb,enabled_modules),require_cash_register=COALESCE(${typeof b.require_cash_register === 'boolean' ? b.require_cash_register : null},require_cash_register),active=CASE WHEN ${status}::text IS NOT NULL THEN ${!['suspenso','cancelado'].includes(status)} ELSE COALESCE(${typeof b.active === 'boolean' ? b.active : null},active) END,status=COALESCE(${status},status),plan_id=CASE WHEN ${planId !== undefined} THEN ${planId || null}::uuid ELSE plan_id END,subscription_due_date=COALESCE(${b.subscription_due_date || null}::date,subscription_due_date),user_limit=COALESCE(${b.user_limit || null},user_limit),product_limit=COALESCE(${b.product_limit || null},product_limit),unit_limit=COALESCE(${b.unit_limit || null},unit_limit),updated_date=now() WHERE id=${id} RETURNING *),log AS (INSERT INTO nexo.market_change_log(market_id,actor_id,action,details) SELECT id,${user.id},'mercado_atualizado',jsonb_build_object('fields',${JSON.stringify(Object.keys(b || {}))}::jsonb) FROM changed) SELECT * FROM changed`;
      if (market && planId) await sql`WITH selected_plan AS (SELECT * FROM nexo.plans WHERE id=${planId}),closed AS (UPDATE nexo.subscriptions SET status='cancelada',cancelled_at=now(),cancellation_reason='Troca de plano',updated_date=now() WHERE market_id=${id} AND status IN ('teste','ativa','inadimplente','suspensa')) INSERT INTO nexo.subscriptions(market_id,plan_id,status,monthly_price,current_period_ends_at) SELECT ${id},id,'ativa',monthly_price,now()+interval '1 month' FROM selected_plan`;
      if (market && status) {
        const subscriptionStatus = status === 'cancelado' ? 'cancelada' : status === 'suspenso' ? 'suspensa' : status === 'teste' ? 'teste' : 'ativa';
    ...
    if (req.method === 'DELETE') {
      const id = path[1];
      if (!isUuid(id)) return send(res,400,{ message:'Mercadinho inválido.' });
      const reason = text(req.body.reason,500) || 'Encerramento pelo Super Admin';
      const [market] = await sql`WITH changed AS (UPDATE nexo.markets SET active=false,status='cancelado',updated_date=now() WHERE id=${id} AND status<>'cancelado' RETURNING *),subscriptions AS (UPDATE nexo.subscriptions SET status='cancelada',cancelled_at=now(),cancellation_reason=${reason},updated_date=now() WHERE market_id=${id} AND status IN ('teste','ativa','inadimplente','suspensa')),log AS (INSERT INTO nexo.market_change_log(market_id,actor_id,action,details) SELECT id,${user.id},'mercado_encerrado',jsonb_build_object('reason',${reason}) FROM changed) SELECT * FROM changed`;
      return send(res,market?200:404,market || { message:'Mercadinho não encontrado ou já encerrado.' });
    }
  }
  if (path[0] === 'users' && req.method === 'POST') {
    ...
    const [capacity] = await sql`SELECT market.user_limit,(SELECT count(*)::int FROM nexo.users WHERE market_id=market.id AND active) AS user_count FROM nexo.markets market WHERE market.id=${user.market_id}`;
    if (capacity?.user_limit && Number(capacity.user_count) >= Number(capacity.user_limit)) return send(res,409,{ code:'USER_LIMIT_REACHED',message:'O limite de usuários do plano foi atingido.' });
  ...
  if (path[0] === 'stock' && path[1] === 'import' && req.method === 'POST') {
    ...
    const [marketCapacity] = await sql`SELECT product_limit FROM nexo.markets WHERE id=${user.market_id}`;
    ...
    const newProductCount = cleanProducts.filter(product => !product.id).length;
    if (marketCapacity?.product_limit && existingRows.length + newProductCount > Number(marketCapacity.product_limit)) return send(res,409,{ code:'PRODUCT_LIMIT_REACHED',message:`A importação ultrapassa o limite de ${marketCapacity.product_limit} produtos deste mercadinho.` });
  ...
  if (path[0] === 'entities') {
    ...
    if (req.method === 'POST') {
      ...
      if (table === 'products') {
        ...
        const [capacity] = await sql`SELECT market.product_limit,(SELECT count(*)::int FROM nexo.records WHERE market_id=market.id AND entity='products') AS product_count FROM nexo.markets market WHERE market.id=${user.market_id}`;
        if (capacity?.product_limit && Number(capacity.product_count) >= Number(capacity.product_limit)) return send(res,409,{ code:'PRODUCT_LIMIT_REACHED',message:'O limite de produtos do plano foi atingido.' });
      }
