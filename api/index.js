...
async function routeHandler(req, res) {
  ...
  if (path[0] === 'sales' && path[1] === 'complete' && req.method === 'POST') {
    ...
    const [sale] = await sql`
      WITH number AS (
        ...
        UPDATE nexo.records product SET data=jsonb_set(product.data,'{quantity}',to_jsonb(COALESCE((product.data->>'quantity')::numeric,0)-source.qty))
  ...
  if (path[0] === 'entities') {
    ...
    if (req.method === 'DELETE') { await sql`DELETE FROM nexo.records WHERE id=${id} AND market_id=${user.market_id} AND entity=${table}`; return send(res,200,{ok:true}); }
  }
